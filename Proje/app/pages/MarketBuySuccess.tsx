...
export function MarketBuySuccess() {
  ...
  const [orderInfo, setOrderInfo] = useState<FragmentOrderInfo | null>(null);
  ...
  useEffect(() => {
    ...
    const raw = sessionStorage.getItem(`toss_fragment_${orderId}`);
    if (!raw) {
      setErrorMsg("주문 정보를 찾을 수 없습니다. 결제는 완료됐을 수 있으니 콜렉션을 확인해 주세요.");
      setStatus("error");
      return;
    }

    const info: FragmentOrderInfo = JSON.parse(raw);
    setOrderInfo(info);

    const token = localStorage.getItem("auth_token") ?? "";

    fetch(`${import.meta.env.VITE_API_URL}/api/market/toss-confirm`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization:  `Bearer ${token}`,
      },
      body: JSON.stringify({ paymentKey, orderId, amount, listingId: info.listingId }),
    })
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
          sessionStorage.removeItem(`toss_fragment_${orderId}`);
          setStatus("done");
        } else {
      ...
      .catch(() => {
        setErrorMsg("서버 통신 오류가 발생했습니다. 콜렉션 페이지에서 결과를 확인해 주세요.");
        setStatus("error");
      });
    ...
    return (
      <div className="page-shell flex items-center justify-center min-h-[50vh]">
        ...
        >
          ...
          <Button className="rounded-2xl bg-[#1456a0] text-white" onClick={() => navigate("/market")}>
            거래소로 돌아가기
          </Button>
        </div>
      </div>
