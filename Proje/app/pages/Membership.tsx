...
function TierIcon({ tier }: { tier: Tier }) {
  ...
  if (tier.name === "일반") return <FaBaseballBall className="h-7 w-7 drop-shadow-sm" style={iconStyle} />;
  if (tier.name === "브론즈") {
    return (
  ...
  if (tier.name === "골드") return <FaTrophy className="h-7 w-7" style={iconStyle} />;
...
function MembershipBadge({ tier }: { tier: Tier }) {
  ...
  return (
    <div className={`group relative flex flex-col items-center gap-3 outline-none`} tabIndex={0} aria-label={`${tier.name} 등급 혜택 보기`}>
