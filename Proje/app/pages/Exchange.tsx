...
export function Exchange() {
  ...
  const fetchStatus = () => {
    fetch(`${API_BASE}/api/exchange/status`, { headers: apiHeaders() })
      .then(r => r.json())
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`${API_BASE}/api/exchange/status`, { headers: apiHeaders() }).then(r => r.json()),
    ]).then(([inv, st]) => {
      setCards(inv.cards ?? []);
      setRaffleCount(inv.raffleTickets ?? 0);
  ...
  const handleExchange = async (card: UserCard) => {
    setExchanging(true);
    try {
      const res = await fetch(`${API_BASE}/api/exchange/nft`, {
        method: "POST",
        headers: apiHeaders(),
        body: JSON.stringify({ cardId: card.id, delivery }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "교환 신청 실패");
      setExchangedIds(prev => new Set(prev).add(card.id));
      setCards(prev => prev.filter(c => c.id !== card.id));
      setConfirmModal(null);
      fetchStatus();
      showToast(`${card.name} 교환 신청이 완료되었습니다!`, "success");
    } catch (err) {
      showToast(err instanceof Error ? err.message : "교환 신청 중 오류가 발생했습니다.", "error");
    } finally {
      setExchanging(false);
    }
  };
  ...
  const handleBuyRaffle = async (pkg: typeof RAFFLE_PACKAGES[0]) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/exchange/buy-raffle`, {
        method: "POST",
        headers: apiHeaders(),
        body: JSON.stringify({ count: pkg.count }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "구매 실패");
      setPoints(prev => prev - pkg.price);
      setRaffleCount(data.newCount ?? (raffleCount ?? 0) + pkg.count);
      fetchStatus();
      showToast(`응모권 ${pkg.count}장을 구매했습니다!`, "success");
    } catch (err) {
      showToast(err instanceof Error ? err.message : "구매 중 오류가 발생했습니다.", "error");
    } finally {
                ...
                <div className="px-6 py-6 space-y-5">
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <input style={inputStyle} placeholder="아파트 동/호수, 층 등" value={delivery.addressDetail}
                  ...
                  <div className="rounded-[14px] px-4 py-4 space-y-2" style={{ background: isDark ? "rgba(37,99,235,0.08)" : "#eef3ff", border: `1px solid ${isDark ? "rgba(37,99,235,0.2)" : "#c0d0f5"}` }}>
                    {[
                      "교환 신청 후 NFT 카드는 즉시 소각됩니다",
                      "실물 배송은 영업일 기준 5~10일 소요됩니다",
                      "교환 신청 후 취소가 불가능합니다",
                    ].map(notice => (
                      <div key={notice} className="flex items-start gap-2">
                        <span className="text-[0.72rem] font-black mt-0.5" style={{ color: accentBlue }}>•</span>
